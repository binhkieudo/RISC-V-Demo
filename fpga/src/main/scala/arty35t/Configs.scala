...
class WithSerialSystemModifications extends Config((site, here, up) => {
  case DTSTimebase => BigInt{(1e6).toLong}
  case BootROMLocated(x) => up(BootROMLocated(x), site).map{ p =>
    val freqMHz = (site(SystemBusKey).dtsFrequency.get / (1000 * 1000)).toLong
    // Make sure that the bootrom is always rebuilt
...
class WithSerMemArty35TTweaks extends Config(
  // Clock configs
  new chipyard.harness.WithAllClocksFromHarnessClockInstantiator ++
  new chipyard.harness.WithHarnessBinderClockFreqMHz(50) ++
  new chipyard.config.WithMemoryBusFrequency(50.0) ++
...
class MyRocketArty32TConfig extends Config(
  new WithTinyArty35TTweaks ++
  new chipyard.config.WithBroadcastManager ++
  new chipyard.TinyRocketConfig
)

class FourRocketArty35TConfig extends Config(
  new WithSerMemArty35TTweaks ++
  new chipyard.config.WithBroadcastManager ++
  new chipyard.FourRocketConfig
)