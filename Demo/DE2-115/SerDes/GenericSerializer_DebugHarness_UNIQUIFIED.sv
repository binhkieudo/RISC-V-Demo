...
module GenericSerializer_DebugHarness_UNIQUIFIED(
  input         clock,
                reset,
                io_in_valid,
  ...
  reg  [4:0]   sendCount;	// @[Counter.scala:61:40]
  wire         wrap_wrap = sendCount == 5'h14;	// @[Counter.scala:61:40, :73:24]
  ...
  always @(posedge clock) begin
    if (_T_2)	// @[Decoupled.scala:51:35]
      data <= {8'h0, data[163:8]};	// @[Serdes.scala:173:17, :187:37]
    ...
    if (reset) begin
      ...
      sendCount <= 5'h0;	// @[Counter.scala:61:40]
    end
    else begin
      ...
      if (_T_2) begin	// @[Decoupled.scala:51:35]
        if (wrap_wrap)	// @[Counter.scala:73:24]
          ...
          sendCount <= sendCount + 5'h1;	// @[Counter.scala:61:40, :77:24]
      end
    end
  end // always @(posedge)
  `ifndef SYNTHESIS
    ...
    initial begin
      ...
      `ifdef RANDOMIZE_REG_INIT
        ...
        sendCount = _RANDOM_5[9:5];	// @[Counter.scala:61:40, Serdes.scala:173:17]
      `endif // RANDOMIZE_REG_INIT
    end // initial
  ...
  assign io_out_bits = data[7:0];	// @[Serdes.scala:173:17, :180:22]
endmodule
